...
| 6 | **Screenshots** 2880×1800 | ✅ **DONE (S50)** — **7** ASC-ready shots in `docs/store/screenshots/`: cup hero / **menu-bar popover (CORE keep-awake feature)** / triggers / general / **Activity charts** / first-run language picker / about (English UI, caramel, sRGB, no-alpha). Owner asked to include the core menu-bar feature + more breadth — done. Owner: **review framing** before upload; reorder in ASC. Regenerate via `screenshots/_scripts/`. | owner review |
