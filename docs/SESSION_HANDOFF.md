...
**Last session:** S50 (2026-05-30) — **whole-app audit (Opus 4.8) + 5 fixes, THEN App Store prep.** (1) 4 parallel review agents over ~11k LOC → **0 CRITICAL**; fixed every MEDIUM+ finding (T1–T5), TDD per fix. **681 → 691 tests.** (2) Owner-requested App Store push: generated **8 ASC-ready 2880×1800 screenshots** (incl. core menu-bar awake state + Activity charts + 6-accent grid + language picker), wired **signing** (ADP enrolled → `DEVELOPMENT_TEAM=4BXCVHZANL`, export-compliance key, signed build verified). (3) Surfaced the **Individual-account → personal-name-on-store** issue → owner will pick a **상호 (brand)** for a later Org-account transfer; **publishing is on hold until the 상호 + bundle-ID decision** (see "DECISION PENDING" below). **⚠️ tests currently pty-gated (trap #9 — reboot/relogin).**
...
**A. (OWNER-driven) App Store upload — Claude side is essentially DONE.** Signing wired (`DEVELOPMENT_TEAM=4BXCVHZANL`, export-compliance key set, signed build verified), **8 screenshots** generated, all `docs/store/` metadata + URLs (200) ready. Remaining is pure owner GUI: App ID register · ASC app record + paste `docs/store/*` + screenshots + $2.99 + App Privacy(none) + age(4+) · Xcode archive→upload→submit · Small Business opt-in.

**⚠️ BUT — do NOT publish yet. One decision blocks first launch (see "DECISION PENDING: brand / bundle ID" below).** The owner is choosing a **상호 (business trade name)** to allow a future switch to an Organization account (so the App Store seller shows the brand, not the personal legal name). Because the **bundle ID is permanent once published**, the brand name must be decided BEFORE the first submission so the bundle ID can be set brand-aligned now. Owner will provide the 상호 in a later session.
...
Resolved this session: account is **Individual** (cert O=Byeong Jun Park) → App Store seller would show the **personal legal name "Byeong Jun Park"**, not a brand. To show a brand instead, the owner needs an **Organization account**, which requires a **Korean 개인사업자(or other) registration + D-U-N-S** under a chosen **상호**. An Organization account then displays the **상호** (not the rep's name).

**Plan agreed: ship v1.0 as Individual now, App-Transfer to an Org account later** (the transfer keeps reviews/ratings/users; built-in ASC flow, not complex). **Two hard constraints make the 상호 a pre-publish decision:**
1. **Bundle ID is permanent once published.** Currently `com.parkbyeongjun.latte` (personal). The owner is undecided on the 상호, and wants the bundle ID brand-aligned (e.g. `com.<brand>.latte`) BEFORE first submission — so **hold publishing until the 상호 is chosen**. Owner will provide it in a later session.
2. **App Transfer is blocked for apps that use iCloud.** Latte's iCloud sync is Phase 2 (still dark). → **the Individual→Org transfer must happen during v1.x, BEFORE iCloud (Phase 2) is ever activated.** Do not flip `LATTE_ICLOUD_SYNC` / ship iCloud until after the brand transfer, or the easy transfer path is lost.

**Next action when owner returns with the 상호:** (a) set `bundleIdPrefix` + `PRODUCT_BUNDLE_IDENTIFIER` (+ `.tests`) in `project.yml`, update `Configuration/*.entitlements`, `Resources/Info.plist` if needed, `.smoke/config.yml` bundle_id, `docs/store/*` references, regenerate, verify signed build; (b) optionally set `NSHumanReadableCopyright` to `© 2026 <brand>`; (c) then owner publishes as Individual; (d) Org transfer later, pre-iCloud. NOTE the `screenshots/_scripts/` + smoke harness + `defaults`/container paths all hardcode `com.parkbyeongjun.latte` — grep and update them on the bundle-ID change.
...
| 4 | **Team ID** → `DEVELOPMENT_TEAM` | ✅ **DONE (S50)** — ADP enrolled; Team ID `4BXCVHZANL` (resolved from the installed Apple Development cert, O=Byeong Jun Park) wired into `project.yml`, regenerated; signed Debug build verified. Also added `ITSAppUsesNonExemptEncryption=false` (skips the per-upload export question). Automatic signing creates the distribution cert when owner archives. | ✅ Claude |
