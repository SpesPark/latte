...
**Shipped in v1.3.1** (S15, 2026-05-01 — same-day continuation of S14, C-3 deferred polish):
- ✅ **C-3 deferred F** Customisable retention window — `ActivityLogStore.retention` becomes mutable via `setRetention(_:)` with immediate GC + flush on shrink. Range 1…90 days, default 14. Wired through `SettingsKey.activityRetentionDays` + `AppEnvironment.activityRetentionDays` published mirror; ActivityTab gains a Stepper. Heatmap title + frame size now follow the user's setting (no more hard-coded 14-day frame).
- ✅ **C-3 deferred B** Per-trigger filter UI — `ActivityFilter` enum (.all / .only(triggerId)) gates the entry stream into both charts. Picker domain is dynamic (only triggers actually present in the snapshot appear), so a fresh user without Schedule entries doesn't see "Schedule" in the menu.
- ✅ **C-3 deferred C** CSV/JSON export — pure `ActivityLogExporter` (no FS / AppKit). CSV uses ISO8601 timestamps for spreadsheets; JSON uses the same `secondsSince1970` Double coder as on-disk so an export round-trips losslessly. NSSavePanel side-effect kept at the call site so `ExportButtons` stays a pure SwiftUI primitive. Filename format `latte-activity-YYYY-MM-DD.{csv,json}` (UTC-based).
- ✅ **C-3 deferred D** Click-row → trigger config jump — Activity "Currently active" rows are buttons with chevron affordance; click flips Settings tab to `.triggers` and `ScrollViewReader` scrolls the matching `TriggerSection` into view. Same path as deep link `latte://settings/triggers?focus=wifi`. `SettingsURLHandler.parseRoute(_:)` returns `(tab, focusedTriggerId)`; legacy `parse(_:)` kept as a thin wrapper. Already-open Settings window re-emits the focus signal via `Notification.Name.settingsRequestFocusTrigger`.
...
**Shipped in v1.9** (S20, 2026-05-02 — second owner-driven manual smoke session, three-commit chain):
- ✅ **P1 — Settings window minHeight 360 → 420** — owner reported during step 7 manual smoke that entering About tab made the tab bar disappear and other tabs unreachable. About content (hero card ~196pt + statusCard ~112pt + spacers + footer + padding) totalled ~386pt minimum vs ~332pt available content area on a stock 460×360 window (after ~28pt tab bar), so the greedy `Spacer()` compressed to negative and content rendered over the tab bar. Extracted `SettingsRoot.minWindowWidth` (460) + `minWindowHeight` (420) as static `CGFloat` constants used by both `SettingsRoot.body` `.frame(...)` and `SettingsWindowController.setContentSize`. Window remains user-resizable above the floor. NEW `Tests/SettingsRootLayoutTests.swift` (2 tests): `XCTAssertEqual` two-way contract pin on both constants. (commit `97e6a1f`)
- ✅ **P2 — dynamic Theme.Colors.cup + foam for light-mode visibility** — owner reported during step 6 popover smoke that both the cup outline AND the rising steam particles were nearly invisible in light mode (after switching from dark mode). Root cause: hard-coded near-white creamy values (`cup` `(0.95, 0.95, 0.97)`, `foam` `(0.96, 0.93, 0.85)`) tuned for dark popover/Settings backdrops; light mode uses a near-white backdrop too, so contrast ≈ 0. Converted both to dynamic `NSColor(name:)` resolving via `appearance.bestMatch(from: [.aqua, .darkAqua])` — pattern matches `CoffeeAccent.color` exactly. Dark mode preserves legacy creamy palette (no regression for existing users). Light mode uses deeper cappuccino tones — cup `(0.42, 0.32, 0.20)` (rich brown body), foam `(0.78, 0.68, 0.50)` (warm tan steam, lighter than cup so foam stroke still reads on liquid layer). NEW `Tests/ThemeAppearanceTests.swift` (initially 6, simplified to 4 after pass 14): legacy dark-mode pinning + light-mode luminance ceilings (cup avg ≤ 0.55, foam avg ≤ 0.7). (commit `654c847`)
- ✅ **14th simplify-pass** APPROVE-WITH-NITS — 0 CRIT/HIGH/1 MED/2 LOW, all addressed: MED-1 `XCTAssertGreaterThanOrEqual` → `XCTAssertEqual` for two-way height contract (matches the existing width test pattern); LOW-1 `foam` + `cup` providers extracted `let isDark = ...` to mirror `CoffeeAccent.color` exactly; LOW-2 deleted two redundant `XCTAssertNotEqual` tests strictly subsumed by the legacy-pinning + luminance-ceiling pair. 556 → **554 tests** (-2 dropped redundant). (commit `fd5a5bc`)
- 548 → **554 tests** (+6 net: +2 layout, +6 appearance, -2 redundant). Smoke unchanged at 22 — both surfaces covered by owner manual smoke steps 6 (popover header) + 7 (Settings tabs).

**Patterns established this session**:
- **Greedy-Spacer-overflow risk** in fixed-height windows. A `VStack { ...fixed content... Spacer() ...footer... }` pattern silently compresses the Spacer to negative when the parent is shorter than fixed content, rendering content out of bounds. Fix: extract the parent minHeight as a named static constant and write an equality test (two-way contract — prevents both shrinking below the empirical floor and silent over-constraining via accidental upward bumps).
- **Dual-mode dynamic NSColor pattern for hard-coded brand colors**. `Color(nsColor: NSColor(name:) { appearance in let isDark = appearance.bestMatch(...) == .darkAqua; return isDark ? legacyValue : derivedDeeperValue })`. Preserves the legacy look in the original mode (no regression) while giving a contrasting variant in the new mode. The `name:` parameter caches the resolution so the closure is called once per appearance change, not per draw frame.
- **Luminance-average proxy as a contrast smoke test**. `(r + g + b) / 3 ≤ threshold` is a coarse but reliable assertion that a color reads against a near-white (or near-black) backdrop without invoking full WCAG contrast math. Useful when the test fixture only has access to the resolved RGB, not the actual rendering context.

**Shipped in v1.8** (S19, 2026-05-02 — first owner-driven manual smoke session, six-commit chain):
- ✅ **Wi-Fi caption non-breaking hyphen fix** — `Pause triggers` caption "Ignore Calendar / App / Wi-Fi / Schedule votes." was wrapping mid-word at the hyphen on narrow popover widths ("wi-/fi"). SwiftUI treats `-` as a soft break point. Replaced with `\u{2011}` (U+2011 NON-BREAKING HYPHEN) so wrap falls only on whitespace boundaries. Owner-reported during S19 step 6 manual smoke. UI-only, 0 test Δ. (commit `11789c3`)
- ✅ **C-7 built-in seed-then-mutable redesign** — owner-reported: hard-coded `QuickPreset.allCases` rows (until 5 PM / 11 PM / midnight) waste popover space for users who never use them. Hard-coded enum cases are by definition not deletable. Redesigned: built-ins now seed once into the regular `recurringQuickPresets` list at first launch and become editable / deletable like any user preset. New `RecurringQuickPreset.builtinSeeds()` returns the three legacy rows with stable hard-coded UUIDs (replay-safe writes), every-day weekdays (`{1...7}`), matching target hours 17/23/0 (midnight uses hour 0 — `nextOccurrence` lookahead rolls forward). New `seedBuiltinPresetsIfNeeded(in:)` migration handles three launch-time cases via the new `SettingsKey.didSeedBuiltinPresets` sentinel: fresh install (sentinel false + empty list → seed), v1.7 upgrader (sentinel false + non-empty → preserve, set sentinel only), returning user (sentinel true → guard returns immediately even if list is empty — owner-cleared state preserved). `MenuBarRoot.swift` drops `ForEach(QuickPreset.allCases)`; popover renders only from `recurringQuickPresets`, gated on `if !activePresets.isEmpty` so a fully-cleared state shows zero divider clutter. `QuickPreset` + `QuickPresetRow` deprecated via doc-comment, retained for API stability + `nextOccurrence` / `minutes` regression tests. Spec §11 added to 08-c7. (commits `7c97c60` + `efb134e`)
- ✅ **12th simplify-pass** APPROVE — 0 CRIT/HIGH/1 MED/2 LOW, all addressed: `seedBuiltinPresetsIfNeeded` doc reworded to "Three launch-time cases" matching §11 table, alignment whitespace on builtinSeed*ID constants normalised, casing typo on `…ResEED…` test method renamed. Bonus regression test pinned the third launch-time case (sentinel-true + non-empty → no-op). 538 → 547 → 548 tests (+10). (commit `c4c9894`)
- ✅ **Delete button in preset edit sheet** — owner-reported: right-click Delete on the General list is too easy to miss. Users tap a row to edit and never discover the context menu. Added bottom-leading destructive Delete button in `RecurringQuickPresetSheet`, only renders in edit mode (`onDelete != nil`); the Add sheet has nothing to delete. Confirmation dialog ("Delete <label>? This preset will be removed from the popover.") protects against accidental click on the already-`.destructive` button. Right-click Delete preserved unchanged. (commit `c7d734c`)
- ✅ **13th simplify-pass** APPROVE-WITH-NITS — 0 CRIT/HIGH/1 MED/2 LOW: `confirmTitleLabelCap = 40` + `displayLabelForConfirm` computed var (trims whitespace, falls back to "this preset" for empty, ellipsis-truncates >40 chars) so a 200-char user label can't push the confirm dialog title off-screen. (commit `992a161`)
- 538 → **548 tests** (+10: 9 builtinSeeds + migration + 1 returning-user-with-presets regression). Smoke unchanged at 22 — UX redesigns covered by owner manual smoke step 6.

**Patterns established this session**:
- **U+2011 non-breaking hyphen** for SwiftUI wrap-control on tight captions where word-internal hyphens would otherwise break readability.
- **Stable hard-coded UUIDs** for replay-safe seed migrations (idempotent writes; theoretical re-runs can't double-seed).
- **Sentinel-gated migration with three launch-time cases** distinguishing "never ran" from "user emptied list" — the only way to preserve owner-cleared state across launches.
- **Bottom-leading destructive sheet button + confirmationDialog** pattern for unrecoverable user-data actions; visually separated from the right-side Cancel/Save axis to prevent mis-click on the default action.

**Shipped in v1.7** (S18, 2026-05-01 — same-day continuation of S17, C-3 + C-7 deferred close-out):
- ✅ **C-3 deferred — live polling refresh** — Activity tab auto-updates while open. `TriggerCoordinator.recordActivity` posts `Notification.Name.activityLogDidAppend` on every vote ON / vote OFF / user-explicit stop with active vote. ActivityTab listens via `.onReceive` and reloads the snapshot, debounced 300 ms via `liveReloadTask` (cancel-on-new-event coalesces bursty boot-time fires). Stop with no active vote suppresses the post; nil store suppresses too. Resolves the "Live polling (refresh while tab open)" item in 09-c3 §10. (commit `54f7d7a`)
- ✅ **C-3 deferred — user-customisable Charts colours** — new pure `ActivityChartPalette` (triggerOrder + defaultHex + `color(for:overrides:)` + encode/decode for SettingsStore round-trip). Defaults match v1.3 palette so a user who never touches the picker sees no visual change. New `Color(hex:)` and `Color.hexString` extensions. New `SettingsKey.activityChartColors` stores `[String: String]` (triggerId → "#RRGGBB"); empty map removes the key. ActivityTab gains "Chart colours" Section: 6 ColorPickers + Reset (only shown when at least one override is set). HourlyAwakeChart now derives chart domain/range from the palette. (commit `67b18b3`)
- ✅ **C-7 deferred — per-day-of-week recurring presets** — owner can define custom popover presets like "Until 6 PM, Mon-Fri only". New `RecurringQuickPreset` (Codable, Equatable, Identifiable, Sendable): id / label / targetHour / targetMinute / weekdays:Set<Int>. Pure helpers: `isActiveOn(date:)`, `nextOccurrence(after:)` (today + 7 days ahead, skipping inactive weekdays), `minutes(from:)` (clamped ≥1, mirrors built-in QuickPreset). New `SettingsKey.recurringQuickPresets` + `AppEnvironment.recurringQuickPresets` @Published mirror. MenuBarRoot popover renders user presets after the built-in QuickPresets, filtered by today's weekday. New "Custom presets" Section in General Settings tab: list with tap-to-edit + context-menu delete + "Add preset…" button → sheet (TextField label + hour/minute pickers + 7 weekday chips). Pure `RecurringPresetWeekday` helper (special-cases Weekdays / Weekends / Every day / Never). (commit `0693f3a`)
- ✅ **11th simplify-pass** APPROVE-WITH-NITS (0 CRIT/HIGH, 1 MED, 4 LOW), all 5 actionable findings addressed: stale doc references, cancellation pattern simplification, dead userInfo dropped, `Color(hex:)` whitespace-tolerance latent defect fixed, `nextOccurrence` doc tightened. (commit `f56e10e`)
- 494 → **538 tests** (+44: 5 live-polling + 16 palette + 23 recurring-preset). Smoke unchanged at 22 — new flows are owner manual smoke step 6/7 territory (popover sees recurring rows on configured weekdays; Activity tab Chart colours section).

**C-3 still-deferred** (no further v1.x scope):
- iCloud sync (best designed alongside B1.2 iCloud-chord-sync — schema integration risk if shipped solo)

**C-7 still-deferred** (no further v1.x scope unless owner requests):
- Path B (`.until(Date)` enum case) — clean abstraction, 3.5h cost, every `switch self` over `AwakeDuration` would need a new branch. Retired.
- Path C (`@Published activeQuickPreset` aside) — rationale was "future-proofs C-3"; C-3 has now shipped, so the marginal benefit is gone. Retired.
