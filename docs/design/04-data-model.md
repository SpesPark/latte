...
- **JSON decode failure**: return default, log `.notice`. Do not erase the bad data — leave it for owner inspection if a user sends a `defaults read com.example.latte` dump.
...
The app's UserDefaults plist (`~/Library/Containers/com.example.latte/Data/Library/Preferences/com.example.latte.plist`) is included in:
