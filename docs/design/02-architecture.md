...
- **`os.Logger`** is used everywhere. Each module has a `Logger(subsystem: "com.araforge.latte", category: "<module>")`.
...
| 1.3 | 2026-04-29 | Session 9.6 — **About-tab status card (A-1) + V2-02 reevaluateWatched surface parity**. New file `Sources/UI/Settings/AssertionStatusFormatter.swift` — pure functions (`stateLabel`, `modeLabel`, `reasonLabel`, `powerLabel`) that take primitive args and return display strings; no SwiftUI imports, no actor isolation, so they unit-test directly. AboutTab updated to take `manager: AwakeManager` + `coordinator: TriggerCoordinator` injections (was zero-arg) and renders a Status card using the formatters: State / Mode (when awake) / Reason (using formatter + latest active trigger vote reason from coordinator) / Power (when `requireACForAwake` is on). `SettingsRoot` updated to pass through. Mode label distinguishes "System + display awake" vs "System awake (display may sleep)" so the user can verify which IOPMAssertion type is currently held without `pmset -g assertions`. **V2-02 surface parity**: `CalendarTrigger`, `WiFiTrigger`, and `ScheduleTrigger` each gain a public `reevaluateWatched()` method matching `AppTrigger.reevaluateWatched()`'s S7.9 contract — running-state guard (`pollTask != nil`), no-op when stopped, immediate `pollOnce()`/`evaluate()` when running. The four config forms (`CalendarTriggerConfigForm` × 4 onChange handlers, `WiFiTriggerConfigForm` × 2, `ScheduleTriggerConfigForm`) were unified to call `trigger.reevaluateWatched()` instead of `pollOnce()`/`evaluate()`/the prior `reevaluate()` directly. Behavioral semantics are unchanged from S8b — the user-facing latency was already 0 — but the public surface is now consistent across all four triggers, which simplifies future trigger additions. 20 new tests: `Tests/AssertionStatusFormatterTests.swift` (14) covers all four formatters across awake/asleep × allowDisplaySleep on/off × reason cases × AC-state combinations; `Tests/ReevaluateWatchedTests.swift` (6) covers running-state guard (Calendar / WiFi / Schedule each: stopped → no vote, running + state-flip → transition vote). 360 → **380 tests, all PASS**. No state machine, Trigger protocol, or persistence schema changes. Tracked as ✅ A-1 + ✅ V2-02 in `docs/v2-backlog.md`. |
